...
| Muurschildering | Vrij zoeken op gebouwnaam, plaats en maker (eigen, losstaande Muurschilderingendatabase, geen CEO), plus exacte rijksmonumentnummer-lookup; geen "bekijk alles"-browsemodus (zie `019-muurschilderingen.md`) | Als marker (punt) bij een eigen coördinaat; ontbreekt die, dan toont Doorzoeker de rijksmonument-centroid als fallback in plaats van het object van de kaart te laten verdwijnen | Gebouwnaam, plaats, makers als tekst en de gekoppelde schilderingen; nog geen afbeeldingen (licentie nog niet bevestigd door RCE) |
