...
export default function Home() {
  ...
  return <main>
    ...
    <section className="hero">
      ...
      <nav aria-label="Direct zoeken">Direct zoeken: {["36046", "Woonhuis", "Archeologisch", "Utrecht"].map((term) => <button type="button" key={term} onClick={() => void executeSearch(term)}>{term}</button>)}<button type="button" onClick={() => void browseType("werelderfgoed")}>Werelderfgoed</button><button type="button" onClick={() => void browseType("gezicht")}>Gezichten</button></nav>
