import { fetchWerelderfgoedGeometrie } from "../../../../lib/server/rce-adapter.ts";
...
// Alleen een RCE CHO-werelderfgoed-URI toestaan, geen willekeurige tekst: de
// query interpoleert deze waarde direct in een SPARQL <...>-node, dus een
// waarde met een ">" erin zou een injectie mogelijk maken.
const CHO_URI_PATTERN = /^https:\/\/linkeddata\.cultureelerfgoed\.nl\/cho-kennis\/id\/werelderfgoed\/\d+$/;
// Lazy, één record tegelijk (zie buildWerelderfgoedGeometrieQuery in
...
export async function GET(request: Request) {
  return withRceErrorHandling({ event: "rce.werelderfgoed-geometrie.error" }, async (startedAt) => {
    ...
    const wkt = await fetchWerelderfgoedGeometrie(cho, request.signal);
    return Response.json({ wkt }, {
      headers: {
